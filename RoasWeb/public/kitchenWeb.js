//여기서 test는 차후 음식점 이름-사용자 이름이 될 것, 나중에 test를 userID로 수정해야함
(function()
{
  // Initialize Firebase
  const config = {
  ...
  function getOrder()
  {
    ...
    dbRefOrder.on('child_added', snap => {
      ...
      if(complete.toString() == 'N')
      {
        ...
        +"<th>" +key+ "번 테이블</th>"
        +"<th>"
        +"<button class=\"ui red button\" id=\"cButton_" +key+ "\">"
        +"완료</button>"
        +"</th>"
        +"</tr>"
        +"</thead>";
  ...
  {
    var complete_to_Y =  'Y';
    document.getElementById('cButton_'+data).addEventListener('click', e=>{
      //완료버튼 기능
      var temp_update = {};
      temp_update['/test/order/' + data + '/complete'] = complete_to_Y;
